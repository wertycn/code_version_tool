...
使用示例：
	...
	检查版本描述文件是否满足发布条件：
		F10-CLI check [版本(任务)号]
		F10-CLI c [版本(任务)号]
	
	查看已发布版本：
		F10-CLI prod version
	
	规范commit提交：
		F10-CLI commit [type] [subject]
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".F10-CLI" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".F10-CLI")
	}
